...
| Local learning absorption | partial | `docs/KNOWLEDGE_INTAKE.md`, `engineering_brain/data/local-learnings.yaml`, `engineering_brain learnings` | field_review/adopt gate は実装済み。既存 candidate の実地 adopt は未了 |
...
3. `engineering_brain/data/local-learnings.yaml` の candidate packets と field_review/adopt fail-closed gate は追加済み。既存 packet の adopt 昇格は未了。
...
1. `engineering_brain/data/local-learnings.yaml` の candidate を field_review し、human-gated adopt / hold を判断する（gate CLI は利用可）。
