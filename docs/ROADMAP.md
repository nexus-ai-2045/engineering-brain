...
`engineering-brain` は clean-history recreation 後の live public repo である。legacy `dev-brain` は migration verification 後に削除済みであり、今後の正本はこの repo に一本化する。
...
| R | verification profile / closeout v2 | task に応じた smoke / preflight / evidence schema を返す | done: profile registry + closeout v2 |
