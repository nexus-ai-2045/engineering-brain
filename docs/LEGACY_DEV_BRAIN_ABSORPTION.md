...
The stale `<PROJECTS_ROOT>/dev-brain` clone is not treated as disposable until its local-only commits are classified.
...
- destructive local deletion of the stale clone is allowed only after this ledger and a final diff check show no unabsorbed material.
...
Before deleting `<PROJECTS_ROOT>/dev-brain` or `<PROJECTS_ROOT>/nexus-dev-kernel`, run:
